...
const checkConfig = async (
  ...
  try {
    ...
    const isNoModelError =
      message.includes('No chat model providers found') ||
      message.includes('No chat models found') ||
      message.includes('No embedding models found');
    ...
    toast.error(message || '配置检查失败，请稍后重试。');
    setIsConfigReady(false);
    setHasError(true);
  }
};
...
export const ChatProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const sendMessage: ChatContext['sendMessage'] = async (
    ...
    // 如果模型尚未配置好，则不真正发送请求，给出提示即可。
    if (!isConfigReady) {
      toast.error('当前还没有可用的大模型，请先在设置中完成模型配置。');
      return;
    }
    ...
    // 这样左右两侧的滚动条只在首页展示，不参与对话页布局。
    if (!params.chatId && messages.length <= 1 && chatId) {
    ...
    if (!systemInstructions) {
      systemInstructions =
        languagePref === 'zh'
          ? '请用中文回答所有问题，除非我明确要求其他语言。'
          : 'Please respond in English unless I explicitly ask for another language.';
    }
